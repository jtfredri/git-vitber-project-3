...
This notebook contains numerical solvers for the two–dimensional Cahn–Hilliard
equation using Fourier spectral methods in space and implicit–explicit (IMEX)
time discretizations. The implementation includes verification via the method
of manufactured solutions, convergence studies, and physical diagnostics such
as mass conservation and energy decay.
...
In addition to backward Euler, a three–stage IMEX Runge–Kutta scheme proposed
by Song is implemented. The scheme advances the solution using a combination
of implicit treatment of the linear stiff terms and explicit treatment of the
nonlinear term. Different coefficient sets are tested and compared in terms of
...
The solution is represented on a uniform grid, and derivatives are computed
using fast Fourier transforms (FFTs). Periodic boundary conditions are assumed
...
An analytical solution \(u(x,y,t)\) is prescribed, and the corresponding source
term \(g(x,y,t)\) is computed symbolically. Convergence studies in time are then
performed while keeping the spatial resolution fixed.
...
These quantities are used to verify mass conservation and energy dissipation
